...
- **Métricas:** Accuracy, Macro-F1, Weighted-F1, precisão/revocação por classe, **matriz de confusão**.
...
- **Métricas:** Micro-F1, Macro-F1, **Hamming loss**, **Subset accuracy** (exact-match), precisão/revocação por label.
...
8. **Estágio 1:** predição vs `equivalent_domain` → Accuracy, Macro-F1, matriz de confusão (25 domínios cobertos).
9. **Estágio 2:** predição vs `equivalent_ml_task_labels` → Micro/Macro-F1, Hamming loss, Subset accuracy.
...
|---|---|---|
| 1 — Domínio | Accuracy, Macro-F1, Weighted-F1, confusão | F1 *in-distribution* do paper (XGBoost) |
| 2 — ML task | Micro-F1, Macro-F1, Hamming loss, Subset accuracy | ≈90% F1 do paper (LP+GloVe+LSVC) |
